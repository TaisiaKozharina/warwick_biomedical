...
\title{\textbf{Advanced Deep Learning for Image Processing} 
\\ ADL4IP -- Report for Assignment X}
\author{Student Y}
\date{\today}
...
Always indicate the question number associated with your answer (e.g., ``Exercise 1.4.2'' for the second sub-task of Exercise 1.4). To make your report is readable as a stand alone document, 
...
Always \textbf{attach the complete code} (possibly as a zip archive) together with the report -- but make sure to upload the report as a separate PDF-file, since that allows us to provide feedback directly in Studium/SpeedGrader.
...
For plots, \textbf{always} write a \textbf{caption} explaining what is shown in the plot, and also what can be observed/concluded from the plot. Remember to always have \textbf{labels} on the axes, and (when suitable) a legend (if multiple curves in same figure). Make the plots of a suitable size to be visible (if printed). See Figure \ref{fig:cost_per_iteration} for an example.
